...
meteor build $OUTPUT_PATH --server=https://fittle-hawaii.meteorapp.com
ls $APK_PATH
...
echo "Copy to server"
mv $APK_PATH/app-aligned.apk $APK_PATH/parccoach-production-kph.apk
cp -f $APK_PATH/parccoach-production-kph.apk $DEST_PATH/parccoach-production-kph.apk
echo "Done building android Production KPH"