...
curl -H "Content-Type: application/json" -X POST -d @./data/json/onboarding.silver.json https://$host/serviceapi/participants/add/woof
...
curl -H "Content-Type: application/json" -X POST -d @./data/json/silverReporting28DaysAfterword.json https://$host/serviceapi/scheduledmessages/add/woof
