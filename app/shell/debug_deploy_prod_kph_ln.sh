...
echo "Copy over production settings & mobil-config for KPH"
...
PRODUCTION_GALAXY_HOSTNAME="fittle-hawaii"
...
deploymentCommand=$GALAXY_DEPLOYMENT_STRING" "$galaxyTarget" --settings "$GALAXY_SETTINGS_JSON_PATH" --owner "$GALAXY_OWNER" --debug"

echo "Deploying DEBUG $deploymentCommand"
