...
Meteor.methods({
        ...
        if (user && user.username && user._id) {
            ...
            if (participant) {
                ...
                */
                ...
                if (startDate) {
                    dayOfStudy = DateHelper.daysDiffUTC(startDate);
                }

                let timeTravelInDays = 0;   // Removing this old way of computing current day
                console.log("\nDEBUG reportedGoalToday");
                console.log("dayOfStudy="+dayOfStudy);
                console.log("timeTravelInDays="+timeTravelInDays);
                //console.log("dayOfStudy - timeTravelInDays="+(dayOfStudy - timeTravelInDays));
    ...
    showDetailReport: function (weekNumber, tasks) {
        // Run only if Meteor.user() is set. Logging out causes this autorun to execute, but in that case Meteor.user() is
        // null, which causes errors to be logged.
        let rowHeading = ["progress-upper-body", "progress-lower-body", "progress-balance", "progress-flexibility", "progress-cardio"];
        ...
        for (let ix = 0; ix < tasks.length; ix++) {
            let task = tasks[ix];
            let goalMet = "not reported";
            if (task.reportCreatedAt) {
                goalMet = task.goalMet ? "met" : "not met";
            }
            let scheduledDate = task.scheduledDate;
            if (scheduledDate) {
                report.goalByDay[scheduledDate] = goalMet;
                let followup = Questions.find({
                    taskId: task._id,
                    tag: 'goalmetfollowup'
                }, {sort: {answerDate: -1}}).fetch();
                if (followup.length > 0) {
                    report.detailGoalByDay[scheduledDate] = followup[0].answer
                }
            }
        }
